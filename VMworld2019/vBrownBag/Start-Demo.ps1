...
#"@
  ...
  for ($_i = $Command; $_i -lt $_lines.count; $_i++)
  {
    if ($_lines[$_i].StartsWith("#"))
    {
	    ...
	    if($_Lines[$_i].StartsWith("#! PAUSE"))
            {
	        Write-Host -NoNewLine -Foreground $CommentColor "# (Pausing)"
            }
	    else
            {
    ...
    {
        if(-not $_Lines[$_i]) { continue }

        # Put the current command in the Window Title along with the demo duration
        $_Duration = [DateTime]::Now - $_StartTime
        $Host.UI.RawUI.WindowTitle = "{2}" -f [int]$_Duration.TotalMinutes, [int] $_Duration.Seconds, $($_Lines[$_i])
        Write-Host -NoNewLine $("`n[$_i]PS> ")
...
"@
      ...
      default
	     ...
	     if(-not $_lines[$_i].StartsWith("#"))
  	     {
                 $output = Invoke-Expression $(".{" + $_lines[$_i] + "}| out-host")
	         if($output) { [System.Console]::ReadLine() }
             }
          }
    } # Switch
  } # for
